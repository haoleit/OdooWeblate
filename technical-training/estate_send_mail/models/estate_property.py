...
class EstatePropertyInherited(models.Model):
    ...
    def _check_buyer_email_format(self):
        for record in self:
            # Check if buyer_email is required based on state
            if record.state in ['offer_received', 'offer_accepted', 'sold'] and not record.buyer_email:
                raise ValidationError(_("Buyer Email is required when the property is in 'Offer Received', 'Offer Accepted', or 'Sold' state."))
